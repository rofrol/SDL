...
macro(CheckDLOPEN)
  ...
  if(NOT HAVE_DLOPEN_IN_LIBC)
    ...
    if(HAVE_DLOPEN_IN_LIBDL)
      sdl_private_link_libraries(CMAKE_PC dl)
    endif()
  endif()
...
macro(CheckOSS)
  if(SDL_OSS)
    ...
    if(HAVE_OSS_SYS_SOUNDCARD_H)
      ...
      sdl_glob_private_sources(${SDL3_SOURCE_DIR}/src/audio/dsp/*.c)
      if(NETBSD)
        sdl_private_link_libraries(CMAKE_PC ossaudio)
      endif()
  ...
  if(SDL_ALSA)
    ...
    if(ALSA_FOUND)
      ...
      if(SDL_ALSA_SHARED)
        if(HAVE_SDL_LOADSO)
          ...
          if(ASOUND_LIB AND ASOUND_SHARED)
            sdl_include_directories(SYSTEM PRIVATE $<TARGET_PROPERTY:ALSA::ALSA,INTERFACE_INCLUDE_DIRECTORIES>)
      ...
      if(NOT HAVE_ALSA_SHARED)
        sdl_private_link_libraries(CMAKE ALSA::ALSA PC_REQUIRES alsa)
      endif()
      set(HAVE_SDL_AUDIO TRUE)
    endif()
  else()
...
macro(CheckPipewire)
    if(SDL_PIPEWIRE)
        ...
        if(PKG_PIPEWIRE_FOUND)
            ...
            sdl_glob_private_sources(${SDL3_SOURCE_DIR}/src/audio/pipewire/*.c)
            set(SDL_AUDIO_DRIVER_PIPEWIRE 1)
            sdl_include_directories(SYSTEM PRIVATE ${PKG_PIPEWIRE_INCLUDE_DIRS})
            ...
            if(SDL_PIPEWIRE_SHARED AND PIPEWIRE_0.3_LIB AND HAVE_SDL_LOADSO)
                ...
                sdl_private_link_options(CMAKE_PC ${PKG_PIPEWIRE_LDFLAGS})
            endif()
            set(HAVE_SDL_AUDIO TRUE)
        endif()
    endif()
...
macro(CheckPulseAudio)
  if(SDL_PULSEAUDIO)
    ...
    if(PKG_PULSEAUDIO_FOUND)
      ...
      sdl_glob_private_sources(${SDL3_SOURCE_DIR}/src/audio/pulseaudio/*.c)
      set(SDL_AUDIO_DRIVER_PULSEAUDIO 1)
      sdl_include_directories(SYSTEM PRIVATE ${PKG_PULSEAUDIO_INCLUDE_DIRS})
      ...
      if(SDL_PULSEAUDIO_SHARED AND PULSE_SIMPLE_LIB AND HAVE_SDL_LOADSO)
        ...
        sdl_private_link_options(CMAKE_PC ${PKG_PULSEAUDIO_LDFLAGS})
      endif()
      set(HAVE_SDL_AUDIO TRUE)
    endif()
  endif()
...
macro(CheckJACK)
  if(SDL_JACK)
    ...
    if(PKG_JACK_FOUND)
      set(HAVE_JACK TRUE)
      sdl_glob_private_sources(${SDL3_SOURCE_DIR}/src/audio/jack/*.c)
      set(SDL_AUDIO_DRIVER_JACK 1)
      sdl_include_directories(SYSTEM PRIVATE ${PKG_JACK_INCLUDE_DIRS})
      ...
      if(SDL_JACK_SHARED AND JACK_LIB AND HAVE_SDL_LOADSO)
        ...
        sdl_private_link_options(CMAKE_PC ${PKG_JACK_LDFLAGS})
      endif()
      set(HAVE_SDL_AUDIO TRUE)
    endif()
  endif()
...
macro(CheckSNDIO)
  if(SDL_SNDIO)
    ...
    if(PKG_SNDIO_FOUND)
      set(HAVE_SNDIO TRUE)
      sdl_glob_private_sources(${SDL3_SOURCE_DIR}/src/audio/sndio/*.c)
      set(SDL_AUDIO_DRIVER_SNDIO 1)
      sdl_include_directories(SYSTEM PRIVATE ${PKG_SNDIO_INCLUDE_DIRS})
      ...
      if(SDL_SNDIO_SHARED AND SNDIO_LIB AND HAVE_SDL_LOADSO)
        ...
        sdl_private_link_options(CMAKE_PC ${PKG_SNDIO_LDFLAGS})
      endif()
      set(HAVE_SDL_AUDIO TRUE)
    endif()
  endif()
  ...
  if(SDL_LIBSAMPLERATE)
    ...
    if(SampleRate_FOUND AND TARGET SampleRate::samplerate)
      ...
      if(SDL_LIBSAMPLERATE_SHARED)
        sdl_include_directories(SYSTEM PRIVATE $<TARGET_PROPERTY:SampleRate::samplerate,INTERFACE_INCLUDE_DIRECTORIES>)
        ...
        sdl_private_link_libraries(CMAKE SampleRate::samplerate PC "samplerate")
      endif()
    else()
      ...
      if(HAVE_LIBSAMPLERATE_H)
        ...
        if(SDL_LIBSAMPLERATE_SHARED AND SAMPLERATE_LIB AND HAVE_SDL_LOADSO)
          ...
          sdl_private_link_libraries(CMAKE_PC "samplerate")
        endif()
      endif()
    endif()
  endif()
...
macro(CheckX11)
  ...
  if(SDL_X11)
    ...
    if(X_INCLUDEDIR)
      sdl_include_directories(SYSTEM PRIVATE "${X_INCLUDEDIR}")
    ...
    if(X11_LIB)
      ...
      sdl_glob_private_sources(${SDL3_SOURCE_DIR}/src/video/x11/*.c)
      ...
      if(NOT HAVE_SHMAT_IN_LIBC)
        ...
        if(HAVE_SHMAT_IN_LIBIPC)
          sdl_private_link_libraries(CMAKE_PC "ipc")
        endif()
      ...
      if(SDL_X11_SHARED)
        ...
        if(X11_LIB)
          if(HAVE_X11_SHARED)
            ...
            # FIXME: target_link_directories is discouraged by CMake. Get rid of it.
            sdl_link_directories(PRIVATE "$<BUILD_INTERFACE:${X11_LIBDIR}>")
            sdl_private_link_libraries(CMAKE_PC X11)
          endif()
        endif()
        if(XEXT_LIB)
          if(HAVE_X11_SHARED)
            ...
            sdl_link_directories(PRIVATE "$<BUILD_INTERFACE:${XEXT_LIBDIR}>")
            sdl_private_link_libraries(CMAKE_PC "Xext")
          endif()
        endif()
      else()
          sdl_link_directories(PRIVATE "$<BUILD_INTERFACE:${X11_LIBDIR}>" "$<BUILD_INTERFACE:${XEXT_LIBDIR}>")
          sdl_private_link_libraries(CMAKE_PC "X11" "Xext")
      endif()
      ...
      if(SDL_X11_XCURSOR AND HAVE_XCURSOR_H AND XCURSOR_LIB)
        ...
        if(HAVE_X11_SHARED)
          ...
          sdl_link_directories(PRIVATE "$<BUILD_INTERFACE:${XCURSOR_LIBDIR}>")
          sdl_private_link_libraries(CMAKE_PC "Xcursor")
        endif()
      ...
      if(SDL_X11_XINPUT AND HAVE_XINPUT2_H AND XI_LIB)
        ...
        if(HAVE_X11_SHARED)
          ...
          sdl_link_directories(PRIVATE "$<BUILD_INTERFACE:${XI_LIBDIR}>")
          sdl_private_link_libraries(CMAKE_PC "Xi")
        endif()
      ...
      if(SDL_X11_XFIXES AND HAVE_XFIXES_H AND HAVE_XINPUT2_H AND XFIXES_LIB)
        if(HAVE_X11_SHARED)
          ...
          sdl_link_directories(PRIVATE "$<BUILD_INTERFACE:${XFIXES_LIBDIR}>")
          sdl_private_link_libraries(CMAKE_PC "Xfixes")
        endif()
      ...
      if(SDL_X11_XRANDR AND HAVE_XRANDR_H AND XRANDR_LIB)
        if(HAVE_X11_SHARED)
          ...
          sdl_link_directories(PRIVATE "$<BUILD_INTERFACE:${XRANDR_LIBDIR}>")
          sdl_private_link_libraries(CMAKE_PC "Xrandr")
        endif()
      ...
      if(SDL_X11_XSCRNSAVER AND HAVE_XSS_H AND XSS_LIB)
        if(HAVE_X11_SHARED)
          ...
          sdl_link_directories(PRIVATE "$<BUILD_INTERFACE:${XSS_LIBDIR}>")
          sdl_private_link_libraries(CMAKE_PC "Xss")
        endif()
...
macro(WaylandProtocolGen _SCANNER _CODE_MODE _XML _PROTL)
    ...
    sdl_private_sources("${_WAYLAND_PROT_C_CODE}")
endmacro()
...
macro(CheckWayland)
  if(SDL_WAYLAND)
    ...
    if(WAYLAND_FOUND)
      sdl_link_directories(PRIVATE "$<BUILD_INTERFACE:${PKG_WAYLAND_LIBRARY_DIRS}>")
      sdl_include_directories(SYSTEM PRIVATE ${PKG_WAYLAND_INCLUDE_DIRS})

      set(HAVE_WAYLAND TRUE)
      set(HAVE_SDL_VIDEO TRUE)

      sdl_glob_private_sources(${SDL3_SOURCE_DIR}/src/video/wayland/*.c)

      # We have to generate some protocol interface code for some unstable Wayland features.
      file(MAKE_DIRECTORY "${CMAKE_CURRENT_BINARY_DIR}/wayland-generated-protocols")
      sdl_include_directories(SYSTEM PRIVATE "${CMAKE_CURRENT_BINARY_DIR}/wayland-generated-protocols")
      ...
      if(SDL_WAYLAND_SHARED AND WAYLAND_CLIENT_LIB AND WAYLAND_EGL_LIB AND WAYLAND_CURSOR_LIB AND XKBCOMMON_LIB AND HAVE_SDL_LOADSO)
        ...
        sdl_private_link_libraries(CMAKE_PC ${PKG_WAYLAND_LIBRARIES})
      endif()
      ...
      if(SDL_WAYLAND_LIBDECOR)
        ...
        if(PKG_LIBDECOR_FOUND)
            ...
            sdl_link_directories(PRIVATE "$<BUILD_INTERFACE:${PKG_LIBDECOR_LIBRARY_DIRS}>")
            sdl_include_directories(SYSTEM PRIVATE "${PKG_LIBDECOR_INCLUDE_DIRS}")
            ...
            if(SDL_WAYLAND_LIBDECOR_SHARED AND DECOR_0_LIB AND HAVE_SDL_LOADSO)
              ...
              sdl_private_link_libraries(CMAKE_PC ${PKG_LIBDECOR_LIBRARIES})
            endif()
...
macro(CheckCOCOA)
  if(SDL_COCOA)
    ...
    if(HAVE_COCOA)
      sdl_glob_private_sources("${SDL3_SOURCE_DIR}/src/video/cocoa/*.m")
...
macro(CheckVivante)
  if(SDL_VIVANTE)
    ...
    if(HAVE_VIVANTE_VDK OR HAVE_VIVANTE_EGL_FB)
      ...
      sdl_glob_private_sources("${SDL3_SOURCE_DIR}/src/video/vivante/*.c")
...
macro(CheckPTHREAD)
  ...
  if(SDL_THREADS AND SDL_PTHREADS)
    ...
    if(HAVE_PTHREADS)
      ...
      sdl_private_link_options(CMAKE_PC ${PTHREAD_LDFLAGS})
      ...
      if(HAVE_PTHREADS_SEM)
        sdl_private_sources(${SDL3_SOURCE_DIR}/src/thread/pthread/SDL_syssem.c)
      else()
        sdl_private_sources(${SDL3_SOURCE_DIR}/src/thread/generic/SDL_syssem.c)
      endif()
...
macro(CheckUSBHID)
  ...
  if(HAVE_USBHID)
    ...
    sdl_glob_private_sources(${SDL3_SOURCE_DIR}/src/joystick/bsd/*.c)
    separate_arguments(USB_CFLAGS)
    sdl_compile_options(PRIVATE ${USB_CFLAGS})
    sdl_private_link_libraries(CMAKE_PC ${USB_LIBS})
...
macro(CheckHIDAPI)
  ...
  if(SDL_HIDAPI)
    if(SDL_HIDAPI_LIBUSB)
      ...
      if(PKG_LIBUSB_FOUND)
        ...
        if(HAVE_LIBUSB_H)
          ...
          sdl_include_directories(SYSTEM PRIVATE ${PKG_LIBUSB_INCLUDE_DIRS})
          if(HIDAPI_ONLY_LIBUSB)
            sdl_private_link_libraries(CMAKE_PC ${PKG_LIBUSB_LIBRARIES})
          else()
            # libusb is loaded dynamically, so don't explicitly link to it
    ...
    if(HAVE_HIDAPI)
      if(ANDROID)
        sdl_private_sources(${SDL3_SOURCE_DIR}/src/hidapi/android/hid.cpp)
      endif()
      if(IOS OR TVOS)
        sdl_private_sources(${SDL3_SOURCE_DIR}/src/hidapi/ios/hid.m)
      ...
      if(SDL_JOYSTICK AND SDL_HIDAPI_JOYSTICK)
        ...
        sdl_glob_private_sources(${SDL3_SOURCE_DIR}/src/joystick/hidapi/*.c)
      endif()
    else()
...
macro(CheckRPI)
  if(SDL_RPI)
    ...
    if(SDL_VIDEO AND HAVE_RPI)
      ...
      sdl_glob_private_sources(${SDL3_SOURCE_DIR}/src/video/raspberry/*.c)
      sdl_private_link_libraries(CMAKE_PC ${VIDEO_RPI_LIBRARIES})
      sdl_compile_options(PRIVATE ${VIDEO_RPI_INCLUDE_FLAGS})
      sdl_private_link_options(CMAKE_PC ${VIDEO_RPI_LIBRARY_FLAGS} ${VIDEO_RPI_LDFLAGS})
    endif()
  endif()
endmacro()
...
macro(CheckKMSDRM)
  if(SDL_KMSDRM)
    ...
    if(PKG_KMSDRM_FOUND AND HAVE_OPENGL_EGL)
      sdl_link_directories(PRIVATE "$<BUILD_INTERFACE:${PKG_KMSDRM_LIBRARY_DIRS}>")
      sdl_include_directories(SYSTEM PRIVATE "${PKG_KMSDRM_INCLUDE_DIRS}")
      set(HAVE_KMSDRM TRUE)
      set(HAVE_SDL_VIDEO TRUE)

      sdl_glob_private_sources(${SDL3_SOURCE_DIR}/src/video/kmsdrm/*.c)
      ...
      if(SDL_KMSDRM_SHARED AND HAVE_SDL_LOADSO)
        ...
        sdl_private_link_libraries(CMAKE_PC ${PKG_KMSDRM_LIBRARIES})
      endif()
    endif()
  endif()
endmacro()
