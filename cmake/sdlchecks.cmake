    ...
    if(SDL_PIPEWIRE)
        ...
        if(PKG_PIPEWIRE_FOUND)
            ...
            if(SDL_PIPEWIRE_SHARED AND PIPEWIRE_0.3_LIB AND HAVE_SDL_LOADSO)
                ...
                sdl_link_directories(${PKG_PIPEWIRE_LIBRARY_DIRS})
                sdl_link_options(${PKG_PIPEWIRE_LDFLAGS_OTHER})
                sdl_link_libraries(${PKG_PIPEWIRE_LIBRARIES})
            endif()
            set(HAVE_SDL_AUDIO TRUE)
        endif()
    endif()
  ...
  if(SDL_PULSEAUDIO)
    ...
    if(PKG_PULSEAUDIO_FOUND)
      ...
      if(SDL_PULSEAUDIO_SHARED AND PULSE_SIMPLE_LIB AND HAVE_SDL_LOADSO)
        ...
        sdl_link_directories(${PKG_PULSEAUDIO_LIBRARY_DIRS})
        sdl_link_libraries(${PKG_PULSEAUDIO_LIBRARIES})
        sdl_link_options(${PKG_PULSEAUDIO_LDFLAGS_OTHER})
      endif()
      set(HAVE_SDL_AUDIO TRUE)
    endif()
  endif()
  ...
  if(SDL_JACK)
    ...
    if(PKG_JACK_FOUND)
      ...
      if(SDL_JACK_SHARED AND JACK_LIB AND HAVE_SDL_LOADSO)
        ...
        sdl_link_directories(${PKG_JACK_LIBRARY_DIRS})
        sdl_link_libraries(${PKG_JACK_LIBRARIES})
        sdl_link_options(${PKG_JACK_LDFLAGS_OTHER})
      endif()
      set(HAVE_SDL_AUDIO TRUE)
    endif()
  endif()
  ...
  if(SDL_SNDIO)
    ...
    if(PKG_SNDIO_FOUND)
      ...
      if(SDL_SNDIO_SHARED AND SNDIO_LIB AND HAVE_SDL_LOADSO)
        ...
        sdl_link_directories(${PKG_SNDIO_LIBRARY_DIRS})
        sdl_link_libraries(${PKG_SNDIO_LIBRARIES})
        sdl_link_options(${PKG_SNDIO_LDFLAGS_OTHER})
      endif()
      set(HAVE_SDL_AUDIO TRUE)
    endif()
  endif()
    ...
    if(X11_LIB)
      ...
      if(SDL_X11_SHARED)
        ...
        if(X11_LIB)
          if(HAVE_X11_SHARED)
            ...
            sdl_link_directories("${X11_LIBDIR}")
            sdl_link_libraries(X11)
          endif()
        endif()
        if(XEXT_LIB)
          if(HAVE_X11_SHARED)
            ...
            sdl_link_directories("${XEXT_LIBDIR}")
            sdl_link_libraries(Xext)
          endif()
        endif()
      else()
        sdl_link_directories("${X11_LIBDIR}" "${XEXT_LIBDIR}")
        sdl_link_libraries(X11 Xext)
      endif()
      ...
      if(SDL_X11_XCURSOR AND HAVE_XCURSOR_H AND XCURSOR_LIB)
        ...
        if(HAVE_X11_SHARED)
          ...
          sdl_link_directories("${XCURSOR_LIBDIR}")
          sdl_link_libraries(Xcursor)
        endif()
      ...
      if(SDL_X11_XINPUT AND HAVE_XINPUT2_H AND XI_LIB)
        ...
        if(HAVE_X11_SHARED)
          ...
          sdl_link_directories("${XI_LIBDIR}")
          sdl_link_libraries(Xi)
        endif()
      ...
      if(SDL_X11_XFIXES AND HAVE_XFIXES_H AND HAVE_XINPUT2_H AND XFIXES_LIB)
        if(HAVE_X11_SHARED)
          ...
          sdl_link_directories("${XFIXES_LIBDIR}")
          sdl_link_libraries(Xfixes)
        endif()
      ...
      if(SDL_X11_XRANDR AND HAVE_XRANDR_H AND XRANDR_LIB)
        if(HAVE_X11_SHARED)
          ...
          sdl_link_directories("${XRANDR_LIBDIR}")
          sdl_link_libraries(Xrandr)
        endif()
      ...
      if(SDL_X11_XSCRNSAVER AND HAVE_XSS_H AND XSS_LIB)
        if(HAVE_X11_SHARED)
          ...
          sdl_link_directories("${XSS_LIBDIR}")
          sdl_link_libraries(Xss)
        endif()
...
#FIXME: return variables/target, don't append sources to SDL target
  ...
  if(SDL_WAYLAND)
    ...
    if(WAYLAND_FOUND)
      ...
      if(SDL_WAYLAND_SHARED AND WAYLAND_CLIENT_LIB AND WAYLAND_EGL_LIB AND WAYLAND_CURSOR_LIB AND XKBCOMMON_LIB AND HAVE_SDL_LOADSO)
        ...
        sdl_link_directories(${PKG_WAYLAND_LIBRARY_DIRS})
        sdl_link_libraries(${PKG_WAYLAND_LIBRARIES})
        sdl_link_options(${PKG_WAYLAND_LDFLAGS_OTHER})
      endif()
      ...
      if(SDL_WAYLAND_LIBDECOR)
        ...
        if(PKG_LIBDECOR_FOUND)
            ...
            if(SDL_WAYLAND_LIBDECOR_SHARED AND DECOR_0_LIB AND HAVE_SDL_LOADSO)
              ...
              sdl_link_directories(${PKG_LIBDECOR_LIBRARY_DIRS})
              sdl_link_libraries(${PKG_LIBDECOR_LIBRARIES})
              sdl_link_options(${PKG_LIBDECOR_LDFLAGS_OTHER})
            endif()
...
macro(CheckPTHREAD)
  ...
  if(SDL_THREADS AND SDL_PTHREADS)
    ...
    if(HAVE_PTHREADS)
      ...
      sdl_link_options(${PTHREAD_LDFLAGS})
      ...
      if(PKG_LIBUSB_FOUND)
        ...
        if(HAVE_LIBUSB_H)
          ...
          if(HIDAPI_ONLY_LIBUSB)
            sdl_link_libraries(${PKG_LIBUSB_LIBRARIES})
          else()
            # libusb is loaded dynamically, so don't use sdl_link_libraries
...
macro(CheckRPI)
  if(SDL_RPI)
    ...
    if(NOT VIDEO_RPI_FOUND)
      ...
      set(VIDEO_RPI_LDFLAGS_OTHER "-Wl,-rpath,/opt/vc/lib")
    endif()

    cmake_push_check_state()
    listtostr(VIDEO_RPI_INCLUDE_DIRS VIDEO_RPI_REQ_INCLUDE_FLAGS "-I")
    listtostr(VIDEO_RPI_LIBRARY_DIRS VIDEO_RPI_REQ_LIBRARY_DIRS "-L")
    separate_arguments(VIDEO_RPI_REQ_LIBRARY_DIRS)
    set(CMAKE_REQUIRED_FLAGS "${CMAKE_REQUIRED_FLAGS} ${VIDEO_RPI_REQ_INCLUDE_FLAGS}")
    list(APPEND CMAKE_REQUIRED_LIBRARIES ${VIDEO_RPI_LIBRARIES})
    list(APPEND CMAKE_REQUIRED_LINK_OPTIONS ${VIDEO_RPI_LDFLAGS_OTHER} ${VIDEO_RPI_REQ_LIBRARY_DIRS})
    check_c_source_compiles("
        #include <bcm_host.h>
    ...
    if(SDL_VIDEO AND HAVE_RPI)
      ...
      # FIXME: add sdl_include_directories
      sdl_link_directories(${VIDEO_RPI_LIBRARY_DIRS})
      sdl_link_libraries(${VIDEO_RPI_LIBRARIES})
      sdl_link_options(${VIDEO_RPI_LDFLAGS_OTHER})
    endif()
  endif()
endmacro()
  ...
  if(SDL_KMSDRM)
    ...
    if(PKG_KMSDRM_FOUND AND HAVE_OPENGL_EGL)
      ...
      if(SDL_KMSDRM_SHARED AND HAVE_SDL_LOADSO)
        ...
        sdl_link_directories(${PKG_KMSDRM_LIBRARY_DIRS})
        sdl_link_libraries(${PKG_KMSDRM_LIBRARIES})
        sdl_link_options(${PKG_KMSDRM_LDFLAGS_OTHER})
      endif()
    endif()
  endif()
endmacro()
