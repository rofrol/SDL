...
static int SDLCALL process_testexitCode(void *arg)
{
    ...
    for (i = 0; i < SDL_arraysize(exit_codes); i++) {
        ...
        process = SDL_CreateProcess((const char * const *)process_args, SDL_FALSE);
        SDLTest_AssertCheck(process != NULL, "SDL_CreateProcess()");
        if (!process) {
            goto failed;
        }
        ...
        SDLTest_AssertCheck(wait_result == SDL_TRUE, "SDL_WaitProcess(): Process should have closed immediately");
...
static int SDLCALL process_testInheritedEnv(void *arg)
{
    ...
    SDL_SetEnvironmentVariable(SDL_GetEnvironment(), TEST_ENV_KEY1, test_env_val1, SDL_TRUE);
    ...
    SDL_SetEnvironmentVariable(SDL_GetEnvironment(),TEST_ENV_KEY2, test_env_val2, SDL_TRUE);
...
static int SDLCALL process_testNewEnv(void *arg)
{
    ...
    process_env = SDL_CreateEnvironment(SDL_TRUE);
    SDL_SetEnvironmentVariable(process_env, "PATH", SDL_GetEnvironmentVariable(SDL_GetEnvironment(), "PATH"), SDL_TRUE);
    SDL_SetEnvironmentVariable(process_env, "LD_LIBRARY_PATH", SDL_GetEnvironmentVariable(SDL_GetEnvironment(), "LD_LIBRARY_PATH"), SDL_TRUE);
    SDL_SetEnvironmentVariable(process_env, "DYLD_LIBRARY_PATH", SDL_GetEnvironmentVariable(SDL_GetEnvironment(), "DYLD_LIBRARY_PATH"), SDL_TRUE);
    SDL_SetEnvironmentVariable(process_env, TEST_ENV_KEY1, test_env_val1, SDL_TRUE);
...
static int SDLCALL process_testKill(void *arg)
{
    ...
    SDLTest_AssertPass("About to call SDL_CreateProcess(SDL_TRUE)");
    process = SDL_CreateProcess(process_args, SDL_TRUE);
    if (!process) {
        goto failed;
    }
    ...
    exit_code = 0xdeadbeef;
    SDLTest_AssertPass("About to call SDL_WaitProcess(SDL_FALSE)");
    result = SDL_WaitProcess(process, SDL_FALSE, &exit_code);
    SDLTest_AssertCheck(result == SDL_FALSE, "Process should not have exited yet");

    SDLTest_AssertPass("About to call SDL_KillProcess(SDL_FALSE)");
    result = SDL_KillProcess(process, SDL_FALSE);
    SDLTest_AssertCheck(result == SDL_TRUE, "Process should have exited");

    exit_code = 0;
    SDLTest_AssertPass("About to call SDL_WaitProcess(SDL_TRUE)");
...
static int process_testStdinToStdout(void *arg)
{
    ...
    exit_code = 0xdeadbeef;
    wait_result = SDL_WaitProcess(process, SDL_FALSE, &exit_code);
    SDLTest_AssertCheck(wait_result == SDL_FALSE, "Process should not have closed yet");
