...
#if SDL_ALTIVEC_BLITTERS && HAVE_SETJMP && !SDL_PLATFORM_MACOS && !SDL_PLATFORM_OPENBSD && !SDL_PLATFORM_FREEBSD
...
#ifdef HAVE_SYSCTLBYNAME
        if (SDL_SystemRAM <= 0) {
#if defined(SDL_PLATFORM_FREEBSD) || defined(__FreeBSD_kernel__) || defined(SDL_PLATFORM_NETBSD) || defined(__DragonFly__)
#ifdef HW_REALMEM
